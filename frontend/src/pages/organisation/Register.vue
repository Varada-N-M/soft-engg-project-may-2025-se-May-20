<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-400 via-purple-400 to-pink-300 flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-2xl">
      ...
      <div class="bg-white rounded-3xl shadow-2xl p-8 relative overflow-hidden">
        <!-- Decorative top wave -->
        <div class="absolute top-0 left-0 right-0 h-20 bg-gradient-to-r from-blue-500 to-purple-600 rounded-t-3xl"></div>
        
        <!-- App logo and title -->
        <div class="relative z-10 text-center mb-6 pt-4">
          <div class="w-20 h-20 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full mx-auto mb-4 flex items-center justify-center shadow-lg">
            <BuildingIcon class="w-10 h-10 text-white" />
          </div>
          <h1 class="text-3xl font-bold text-gray-800 mb-2">Register Your Organization</h1>
          <p class="text-gray-600 text-sm">Join LearnFun and start creating amazing learning experiences for kids!</p>
        </div>

        <!-- Response Alert -->
        <div v-if="response" class="mb-6 p-4 rounded-lg border-2" :class="response.success ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'">
...
const handleSubmit = async () => {
  ...
  try {
    const apiResponse = await fetch('http://127.0.0.1:5000/api/organization/register', {
      method: 'POST',
      headers: {
    ...
    if (apiResponse.ok) {
      response.value = {
        success: true,
        message: result.message || 'Organization registered successfully! Welcome to LearnFun! 🎉',
        data: result
      }
